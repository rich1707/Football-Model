...
There aren't many situations in R where loops are needed, but we shall need both a `for()` loop and a `while()` loop to navigate through the pages. Basically, the while-loop runs for as long as the prev_page_button (a JavaScript control) is active, which allows us to access all sub-pages within each url.  
...
football_data %>% 
   ...
   scale_fill_manual(values = c("#a1adb6", "#b0afa0", "#7a8477")) +
   theme_classic() + 
   theme(
...
football_data %>% 
   ...
   guides(fill = guide_legend(reverse = TRUE, title = NULL)) +
   scale_fill_manual(values = c("#a1adb6", "#b0afa0")) +
   theme_classic() +
   theme(
...
football_data %>% 
   ...
   guides(fill = guide_legend(reverse = TRUE, title = NULL)) +
   scale_fill_manual(values = c("black", "ivory4", "orange3")) +
   theme_classic() +
   theme(
      axis.text.x = element_text(face = "bold"),
      axis.text.y = element_text(face = "bold"),
...
One thing to not is how similar the order of the teams is between these plots. It seems that goal-supremacy and the total number of points is highly correlated, so we shall focus on engineering features based on goals. The home-advantage is also significant and needs to be taken into account.
...
Perhaps surprisingly the answer is yes: we make a profit for four out of five seasons. It is probably significant that the one losing season was when the matches were played behind closed doors, which may well have thrown off the implicit home-advantage feature.
